...
For some devices, the user is not allowed to configure the wifi hotspot from the app. Currently for Android versions < 8, “ODK-SKUNKWORKS-CROW-odk-skunkworks-crow” is set as the default network name and for versions >= 8 it is AndroidShare-XXXX which is set by Android.
